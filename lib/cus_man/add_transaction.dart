...
//  =========  فتح نافذة ملخص العمليات ===========
  Future<void> _showSummaryDialog(BuildContext context) async {
    ...
    /*    final summary = await DatabaseHelper().getSummaryByDate(_selectedDate!);
    final summaryAgeent =
        await DatabaseHelper().getAgentSummaryByDate(_selectedDate!);
 */
    // ignore: use_build_context_synchronously
                    ...
                    /*                    TableRow(
                      children: [
                        ...
                        Padding(
                          padding: const EdgeInsets.all(10.0),
                          child: Text(
                            summaryAgeent['balance'].toString(),
                            style: TextStyle(
                              color: summary['balance']! >= 0
                                  ? Colors.green
                                  : Colors.red,
...
/* void _showAddAgentOperationDialog() {
  setState(() {
    matchingAgents = []; // إعادة تعيين القائمة المقترحة
  });
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 10.0),
                  // نفس الحقول الأخرى (المبلغ والتفاصيل) كما في إضافة العميل
                  // يمكنك نسخ نفس الكود من دالة _buildNameFieldWithSuggestions
                ],
              ),
            ),
            const SizedBox(height: 10.0),
              ...
              children: [
                GestureDetector(
                  onTap: () {
                    ...
                    _saveAgentTransaction(); // حفظ عملية الوكيل
                    Navigator.pop(context);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(
...
void _saveAgentOperation() async {
  if (_transactionType.isNotEmpty) {
    ...
    if (_selectedView != 'agents') {
      await _fetchAgentTransactionsByDate(DateTime.now()); // جلب عمليات الوكلاء
      setState(() {
        _selectedView = 'agents'; // تغيير العرض للوكلاء
      });
    }
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
